...
// constructor
// null 和undefined 没有 constructor
...
// 确点：乘以10的幂次方这种方法对大数支持的依然不好

// cookies、sessionStorage 和localStorage的区别
// cookie是网站为了标识用户身份而存储在用户本地中的数据（这些数据通常是经过加密的）
// cookie数据始终在同源的HTTP请求中携带，即使不需要也会进行传递
// sessionStorage 和 localStorage 仅保存在本地，不会将数据发送到服务器
// cookie 的存储大小不会超过4k，而sessionStorage 和 localStorage的存储空间为5M的字符数
// cookie 在过期时间内有效、sessionStorage在关闭浏览器窗口前有效、localStorage持久有效

// cookie 和 token 的区别
// token 支持跨域访问，cookie 不允许跨域访问
...
// 每一个函数都有一个[[Scopes]]属性，其存储的是这个函数运行时的作用域链，除了当前函数的 AO，作用域链的其他部分都会在其父函数预编译时添加到函数的[[Scopes]]属性上（因为父函数也需要预编译后才能确定自己的AO）,所以js的作用域是词法作用域
...
//mdn：闭包是访问自由变量的函数
// 形成私有上下文，保护私有变量的一种机制
// 函数每一次执行都是从新形成一个全新的私有上下文，和之前执行产生的上下文没有必然的联系
// 实际：每一个函数都会产生闭包，无论闭包中是否存在内部函数或者 内部函数中是否访问了当前函数变量又或者是否返回了内部函数，因为闭包在当前函数预编译阶段就已经创建了

// 闭包缺点
/// 由于闭包会使得函数中的变量都保存在内存中，内存消耗很大，所以不能滥用闭包，否则会造成网页的性能问题，在IE中可能导致内存泄漏。解决方法是：在退出函数之前，将不使用的局部变量全部删除
// 闭包会在父函数外部，改变父函数内部变量的值。所以，如果你把父函数当作对象（object）使用，把闭包当作它的公用方法（Public Method）,把内部变量当作它的私有属性（private value）,这时一定要小心，不要随便改变父函数的内部变量的值
...
// WeakMap
// 只接受对象作为健名（null除外），不接受其他类型的值作为键名
...
// Object.keys返回所有属性key,但不包含不可枚举的属性
// Reflect.ownKeys返回所有属性key,包含枚举的属性

// 创建不可变的对象
// Object.freeze 和 Object.seal 区别
// Object.seal 仅保护对象不添加和删除属性，可更新对象属性。
...
// _proto_是一个对象，他有两个属性，constructor和 _proto_
// 原型对象prototype有一个默认的constructor 属性，用于记录实例是由哪个构造函数创建
// Person.prototype.contructor === Person
// person01._proto_ === Person.prototype
// 除了Object的原型对象（Object.prototype）的_proto_指向null，其他内置函数对象的原型对象（例如：Array.prototype)和自定义构造函数的_proto_都指向Object.prototype,因为原型对象本身是普通对象

// 为什么数组长度能任意变化
// length 属性可读可写，大多数其他编程语言不允许直接改变数组的大小，越界访问索引会报错

// defer 和 async有什么区别
// 二者都是异步去加载外部js文件 ，
// async 是在外部js加载完成后，浏览器空闲时，Load事件触发前执行，而defer是在js加载完成后，整个文档解析完成后执行
// defer 更像是将<script>标签放在</body>之后的效果，但是它由于是异步加载JS文件，所以可以节省时间
...
// symbol迭代器
// symbol.asyncIterator 符号指定了一个对象的默认异步迭代器，如果一个对象设置了这个属性，他就是异步可迭代对象，可用于for await...of循环
// symobl.hasInstance用于判断某对象是否为某构造器的实例。
// symobl.iterator 为每一个对象定义了默认的迭代器。
// iterator是一种接口，为各种不同的数据结构提供统一的访问机制（任何数据结构部署iterator接口，就可以完成遍历操作），提供给for...of消费
// 可迭代的对象：Array、Map、Set、String、TypedArray、函数的arguments 对象、NodeList 对象
const arr = [1, 2, 3];
...
// 发布订阅模式
//     订阅者把自己想订阅的事件注册到调度中心，当发布者发布该事件到调度中心，也就是该事件触发时，由调度中心统一调度订阅者到调度中心的事件
//代理模式
// 装饰器模式
