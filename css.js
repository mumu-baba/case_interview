...
// flex 布局
// 在flex 中，最核心的概念就是容器和轴，所有的属性都是围绕容器和轴设置的。其中，容器分为父容器和子容器。轴分为主轴和交叉轴（主轴默认为水平方向，方向向右，交叉轴为主轴顺时针旋转 90°）

// flex父容器属性
// flex-direction: 主轴的方向，默认值row
// flex-wrap: 超出父容器子容器的排列方式，默认值nowrap
// flex-flow: flex-direction 和 flex-wrap 的简写，默认值row nowrap
// justify-content:子容器在主轴的排列方向，默认值flex-start
// align-items:子容器在交叉轴的排列方向默认值stretch
...
// flex-basis: flex-basis给上面两个属性分配多余空间之前，计算项目是否有多余空间，默认值为auto，即项目本身的大小

// css开启GPU加速
// css3 硬件加速也叫GPU加速，是利用GPU进行渲染，减少GPU操作的一种优化方案，由于GPU中的transform等css属性不会触发repaint(重绘)，所以能大大提高网页的性能
// transform
// opacity
// filter
// with-change
...
// 圣杯布局缺点：当center < right 会变形
// 双飞翼布局缺点：多添加一个dom


// bfc
// bfc 是一个独立的布局环境，bfc内部的元素布局与外部互不影响
...
// em和rem 是相对长度单位，更适合响应式布局
// em相对于父元素，rem相对于根元素

// rem适配
// 设置根字体大小再计算
// 设计稿除以100，等分为7.5份来实现移动端不同屏幕尺寸适配的原理
// 个大浏览器厂商，已经支持viewport
// postcss-px-to-viewport-8-plugin: px -> vw
// vue模版中的px单位不会被转换，如需转换请使用postcss-style-px-to-viewport工具

// canvas 和 svg区别
// canvas 是h5标签，svg是矢量图形，放大缩小不会失真由dom组成
// canvas 主要是用笔刷来绘制2D图形
// svg主要是用标签来绘制不规则矢量图
// canvas画的位图，svg画的是矢量图
// svg节点过多时渲染慢，canvas性能更好，但写起来复杂
